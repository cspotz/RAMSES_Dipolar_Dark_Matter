  ...
  if(poisson)then
     do ind=1,twotondim
		...
		if(typep(ind_part(j))%tag==1)then !test if DM
    	          ff(j,idim)=0!ff(j,idim)+(xi_b(indp(j,ind),idim)-F_int(indp(j,ind),idim))*vol(j,ind) !DM move according to xi_b (=F_t) - F_int
...
!!!!!!!!!!!!!!!!!!!!!!!!
!7/12/21 leapfrog for Pi
  if(poisson)then
     do ind=1,twotondim
        do idim=1,ndim
           do j=1,np
		if(f2(indp(j,ind),idim)==0)then
...
!7/12/21 leapfrog for Pi
  if(poisson)then
     do ind=1,twotondim
        do idim=1,ndim
           do j=1,np
                if(f3(indp(j,ind),idim)==0)then
                                        xi_b_dot(indp(j,ind),idim)=xi_b_dot(indp(j,ind),idim)+pi_dot(indp(j,ind),idim)*dtnew(ilevel)
                       f3(indp(j,ind),idim)=1
                endif
