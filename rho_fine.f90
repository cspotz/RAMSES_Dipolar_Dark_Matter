...
subroutine rho_fine(ilevel,icount)
  ...
  ! Array rho, rhod and cpu_map2 are stored with:
  ! - rho containing the Poisson source term
  ! - rhod containing the Poisson source term
  ...
  !-------------------------------------------------------
  ! Initialize rho and rhod to analytical and baryon density field
  !-------------------------------------------------------
  if(ilevel==levelmin.or.icount>1)then
     do i=nlevelmax,ilevel,-1
        ...
        ! Update boundaries
        	...
        	call make_virtual_reverse_dp(rhod(1),i) ! negative mass
        	call make_virtual_fine_dp   (rhod(1),i) ! negative mass
     end do
  end if
	...
	phi(active(ilevel)%igrid(i)+iskip)=0.0D0
     ...
     if(ilevel==cic_levelmax)then
	   ...
	   rho_top(active(ilevel)%igrid(i)+iskip)=0.0D0
           rhod_top(active(ilevel)%igrid(i)+iskip)=0.0D0 !negative mass
        end do
     endif
  end do
  if(cic_levelmax>0.and.ilevel>cic_levelmax)then
     do ind=1,twotondim
        ...
        do i=1,active(ilevel)%ngrid
           ...
           rhod_top(active(ilevel)%igrid(i)+iskip)=rhod_top(father(active(ilevel)%igrid(i))) !negative mass
           rhod(active(ilevel)%igrid(i)+iskip)=rhod(active(ilevel)%igrid(i)+iskip)+ &
                & rhod_top(active(ilevel)%igrid(i)+iskip)
        end do
     end do
  endif
  ...
  do icpu=1,ncpu
     do ind=1,twotondim
        ...
        do i=1,reception(icpu,ilevel)%ngrid
           ...
           rhod(reception(icpu,ilevel)%igrid(i)+iskip)=0.0D0 !negative mass
        end do
        if(ilevel==cic_levelmax)then
           do i=1,reception(icpu,ilevel)%ngrid
              rho_top(reception(icpu,ilevel)%igrid(i)+iskip)=0.0D0
              rhod_top(reception(icpu,ilevel)%igrid(i)+iskip)=0.0D0 !negative mass
           end do
        endif
     end do
  end do
  ...
  call make_virtual_reverse_dp(rhod(1),ilevel) !negative mass
  call make_virtual_fine_dp   (rhod(1),ilevel) !negative mass
  if(ilevel==cic_levelmax)then
	call make_virtual_reverse_dp(rho_top(1),ilevel)     
	call make_virtual_reverse_dp(rhod_top(1),ilevel) !negative mass
  endif
  if(cic_levelmax>0.and.ilevel>=cic_levelmax)then
     call make_virtual_fine_dp   (rho_top(1),ilevel)
     call make_virtual_fine_dp   (rhod_top(1),ilevel) !negative mass
  endif
  if(m_refine(ilevel)>-1.0d0)then
  ...
  do ibound=1,nboundary
     do ind=1,twotondim
        ...
        do i=1,boundary(ibound,ilevel)%ngrid
           ...
           rhod(boundary(ibound,ilevel)%igrid(i)+iskip)=0 !negative mass
        end do
     end do
  end do
  ...
  do ind=1,twotondim
     ...
     if(cic_levelmax==0.or.ilevel<=cic_levelmax)then
		...
		if(typep(ind_part(j))%tag==1)then !test if negative mass
		     rhod(indp(j,ind))=rhod(indp(j,ind))+vol2(j)
     ...
     if(ilevel==cic_levelmax)then
		...
		if(typep(ind_part(j))%tag==1)then
			rhod(indp(j,ind))=rhod(indp(j,ind))+vol2(j) !negative mass
...
subroutine cic_from_multipole(ilevel)
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        rhod(active(ilevel)%igrid(i)+iskip)=0.0D0 !negative mass
     end do
  end do
