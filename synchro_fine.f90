! Stahl 16/06/2021 : modification for dipolar dark matter
...
subroutine sync(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
 ...
 ! Compute parent cell position
  ...
  do ind=1,twotondim
     do idim=1,ndim
        do j=1,np
		if(typep(ind_part(j))%tag==1)then !test if DM
    	          ff(j,idim)=0!ff(j,idim)+(xi_b(indp(j,ind),idim)-F_int(indp(j,ind),idim))*vol(j,ind)/10 !DM move according to xi_b (=F_t) - F_int
		else if(typep(ind_part(j))%tag==0)then
		  ff(j,idim)=ff(j,idim)+(xi_b(indp(j,ind),idim)+xi_b_dot(indp(j,ind),idim))*vol(j,ind) !baryon ////ff(j,idim)=0!static
		end if	
        end do
     end do
  end do
...
!!!!!!!!!!!!!!!!!!!!!!!!
!7/12/21 leapfrog for Pi, Pidot stored in pi_dot, variontion temporelles de xi stored in xi_dot (~pi) variation temporelles de F_int stored in ff2

  if(poisson)then
    do ind=1,twotondim
     do idim=1,ndim
        do j=1,np
          if(f4(indp(j,ind),idim)==0)then    
		pi_dot(indp(j,ind),idim)=pi_dot(indp(j,ind),idim)+(f_int(indp(j,ind),idim)+ff2(indp(j,ind),idim))*rhod(indp(j,ind))*0.5D0*dtnew(ilevel)
		f4(indp(j,ind),idim)=1
	  endif
        end do
    end do
      end do
