...
subroutine load_balance
  ...
  use poisson_commons, ONLY: phi, f, pi_b, f_int, w_pol_p, xi_b
#ifdef RT
  ...
  do ilevel=nlevelmax,1,-1
     ...
     if(poisson)then
        ...
        call make_virtual_fine_dp(w_pol_p(1),ilevel) !dipolar dark matter 29/04/21
        do idim=1,ndim
           call make_virtual_fine_dp(f(1,idim),ilevel)
           call make_virtual_fine_dp(xi_b(1,idim),ilevel) !dipolar dark matter
           call make_virtual_fine_dp(f_int(1,idim),ilevel) !dipolar dark matter
           call make_virtual_fine_dp(pi_b(1,idim),ilevel) !dipolar dark matter
        end do
     end if
  end do
...
!!29/04/2021 Duplicate the previous part for polar
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=real(w_pol_p(iskip2+igrid),kind=qdp)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     w_pol_p(iskip2+igrid)=real(hilbert_key(igrid),kind=8)
  end do
  end do
...
!!29/04/2021 Duplicate the previous part for polar
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=real(xi_b(iskip2+igrid,idim),kind=qdp)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     xi_b(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
  end do
  end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=real(f_int(iskip2+igrid,idim),kind=qdp)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     f_int(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
  end do
  end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=real(pi_b(iskip2+igrid,idim),kind=qdp)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     pi_b(iskip2+igrid,idim)=real(hilbert_key(igrid),kind=8)
  end do
  end do
  end do
