...
!################################################################
  !------------------------------------------------------------------
  ! 08/03/2021
  ! I modify the routines for the dipolar DM
  !------------------------------------------------------------------
...
subroutine make_boundary_force(ilevel)
  ...
  real(dp),dimension(1:nvector,1:ndim),save::cff2

  real(dp),dimension(1:nvector,1:ndim),save::cff3
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           if((boundary_type(ibound)/10).ne.2)then
		    ...
		    cff2(i,ivar)=xi_b(ind_cell_ref(i),ivar)
		    cff3(i,ivar)=f_int(ind_cell_ref(i),ivar)
                 end do
              end do
              ...
              do ivar=1,ndim
                 ...
                 do i=1,ngrid
                    ...
                    xi_b(ind_cell(i),ivar)=cff2(i,ivar)*switch
                    f_int(ind_cell(i),ivar)=cff3(i,ivar)*switch
                 end do
              end do
              ...
              call gravana(xx,cff2,dx_loc,ngrid)
	      call gravana(xx,cff3,dx_loc,ngrid)

              ! Scatter variables
              do ivar=1,ndim
                 do i=1,ngrid
                    f(ind_cell(i),ivar)=ff(i,ivar)
                    xi_b(ind_cell(i),ivar)=cff2(i,ivar)
                    f_int(ind_cell(i),ivar)=cff3(i,ivar)
                 end do
              end do
...
subroutine make_boundary_phi(ilevel)
  ...
  do ibound=1,nboundary
     ...
     do igrid=1,ncache,nvector
        ...
        !   if(ngrid>0) call phi_ana(rr,pp2,ngrid)

           ! Scatter variables
           do i=1,ngrid
              phi(ind_cell(i))=pp(i)/scale
              w_pol_p(ind_cell(i))=pp(i)/scale !dipole 
           end do
