 ...
 ! Wait for the token
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        write(ilun)ncache
        if(ncache>0)then
           allocate(ind_grid(1:ncache),xdp(1:ncache)) !polar
           ...
           do ind=1,twotondim
              ...
              do ivar=1,ndim
                 do i=1,ncache
                    xdp(i)=f_int(ind_grid(i)+iskip,ivar)
                 end do
                 write(ilun)xdp
              end do
              ! Write new force
              do ivar=1,ndim
                 do i=1,ncache
                    xdp(i)=pi_b(ind_grid(i)+iskip,ivar)
                 end do
                 write(ilun)xdp
              end do
               !Write polarization (after all the ndim component of the force)
              do ivar=1,ndim
                 do i=1,ncache
                    xdp(i)=xi_b(ind_grid(i)+iskip,ivar)
                 end do
                 write(ilun)xdp
              end do
		! Write derivative polarization (after all the ndim component of the force)
              do ivar=1,ndim
                 do i=1,ncache
                    xdp(i)=xi_b_dot(ind_grid(i)+iskip,ivar)
                 end do
                 write(ilun)xdp
              end do
           end do
