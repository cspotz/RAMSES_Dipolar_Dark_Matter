...
subroutine make_grid_fine(ind_grid,ind_cell,ind,ilevel,nn,ibound,boundary_region)
  use amr_commons
  use hydro_commons
  use poisson_commons, ONLY:f, phi,phi_old,w_pol_p,xi_b,f_int,pi_b
#ifdef RT
        ...
        do j=1,twotondim
           ...
           do idim=1,ndim
              do i=1,nn
                 ...
                 xi_b(iskip+ind_grid_son(i),idim)=xi_b(ind_fathers(i,0),idim)
                 f_int(iskip+ind_grid_son(i),idim)=f_int(ind_fathers(i,0),idim)
                 pi_b(iskip+ind_grid_son(i),idim)=pi_b(ind_fathers(i,0),idim)
              end do
           end do
           do i=1,nn
              phi(iskip+ind_grid_son(i))=phi(ind_fathers(i,0))
              w_pol_p(iskip+ind_grid_son(i))=w_pol_p(ind_fathers(i,0)) !29/04/2021 Stahl PolarDM
  ...
  do ind=1,twotondim
     ...
     if(poisson)then
        do i=1,nn
           ...
           w_pol_p(ind_cell_son(i))=0.0D0 
           phi_old(ind_cell_son(i))=0.0D0
        end do
        do idim=1,ndim
           do i=1,nn
              f(ind_cell_son(i),idim)=0.0D0
              pi_b(ind_cell_son(i),idim)=0.0D0
              f_int(ind_cell_son(i),idim)=0.0D0
              xi_b(ind_cell_son(i),idim)=0.0D0
           end do
        end do
     end if
     ! Hydro variables
