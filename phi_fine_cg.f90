...
subroutine phi_fine_cg(ilevel,icount)
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        rhs_norm=rhs_norm+fact2*(rho(idx)-rho_tot+0*divpi(idx))*(rho(idx)-rho_tot+0*divpi(idx)) !Poisson equation gets modified due to dipolar;+divpi because grad comes with a minus ; in order to add the non-spherical term, take out the 0*divpi and also in the error later ;  BUG ? check units of divpi (4 order of magnitude > rho atm) !! 
     end do
  end do
  ...
  call make_virtual_fine_dp(w_pol_p(1),ilevel) !dipolar dark matter 08/03/21 !dipolar DM 08/03/21
...
subroutine cmp_residual_cg(ilevel,icount)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           residu(i)=residu(i)+fact*(rho(ind_cell(i))-rho_tot+0*divpi(ind_cell(i))) !dipolar DM ; in order to add the non-spherical term, take out the 0*divpi
        end do
